...
import CreateTourModal from '@/components/tour/create-tour-modal'
...
import { Filter, ArrowUpDown, Search, Upload, Orbit, FolderPlus, Trash2, X, CheckSquare, AlertTriangle, Pencil } from 'lucide-react'
...
function mapRow(p: TourProjectListItem, globalIndex: number): Project {
  ...
  return {
    ...
    stats: {
      likes: p.total_linked_scenes || 0,   // scenes with outgoing links
      comments: 0,                         // tours have no comments
      shares: p.total_hotspots || 0,       // navigation links
      images: p.total_scenes || 0,
    },
  }
}

type ToursDashboardProps = {
  initialProjects: TourProjectListItem[]
  initialTotal: number
  isAdmin: boolean
}

export default function ToursDashboard({ initialProjects, initialTotal, isAdmin }: ToursDashboardProps) {
  ...
  const [sortOrder, setSortOrder] = useState<TourSort>('newest')
  ...
  const nextSortOrder: TourSort = sortOrder === 'newest' ? 'oldest' : sortOrder === 'oldest' ? 'name' : 'newest'

  const refetchFirstPage = useCallback(async (search: string, sort: TourSort) => {
    const seq = ++requestSeq.current
    setIsLoading(true)
    try {
      const { projects: rows, total: newTotal } = await getTourProjectsPage({ page: 1, search, sort })
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const { projects: rows, total: newTotal } = await getTourProjectsPage({ page: nextPage, search: searchQuery, sort: sortOrder })
      if (seq !== requestSeq.current) return
      setProjects(prev => {
  ...
  const handleOpen = (project: Project) => {
    router.push(`/tours/${project.id}?name=${encodeURIComponent(project.title)}`)
  }

  const handleDelete = (projectId: string) => {
    const project = projects.find(p => p.id === projectId)
    setDeleteModal({ type: 'single', projectId, projectName: project?.title ?? 'this tour' })
  }
  ...
  const handleDuplicate = async (project: Project) => {
    ...
    const result = await duplicateTourProject(project.id)
    setIsDuplicating(false)
    if (!result.success) {
      toast({ title: 'Failed to duplicate tour', description: result.error ?? 'Please try again.', variant: 'destructive' })
      return
    }
    toast({ title: 'Tour duplicated', description: `"${project.title}" was copied with all scenes and links.` })
    refresh()
  }
  ...
  const handleConfirmRename = async () => {
    ...
    setIsRenaming(true)
    setRenameError(null)
    const result = await renameTourProject(renameTarget.id, trimmed)
    setIsRenaming(false)
    if (!result.success) {
      setRenameError(result.error ?? 'Failed to rename tour')
      return
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    if (deleteModal.type === 'single') {
      const result = await deleteTourProject(deleteModal.projectId)
      setIsBulkDeleting(false)
      setDeleteModal(null)
      if (result.success) refresh()
    } else {
      const results = await Promise.all(deleteModal.ids.map(id => deleteTourProject(id)))
      const failed = results.filter(r => !r.success).length
      setSelectedIds(new Set())
      setIsBulkDeleting(false)
      setDeleteModal(null)
      if (failed > 0) console.error(`${failed} tour(s) failed to delete.`)
      refresh()
    }
  }

  const handleTourCreated = (projectId: string) => {
    refresh()
    // Jump straight into the editor so scenes can be linked right away.
    router.push(`/tours/${projectId}`)
  }
    ...
    <>
      <div className="p-6 md:p-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-lg font-semibold text-foreground font-display">Virtual Tours</h1>
          <div className="flex items-center gap-2">
            <Tooltip>
            ...
            {isAdmin && (
              <Tooltip>
                ...
                />
                <TooltipContent>Create new tour</TooltipContent>
              </Tooltip>
            )}
          </div>
        </div>
        ...
        {isLoading && projects.length === 0 ? (
          ...
          <div className="flex h-64 flex-col items-center justify-center text-muted-foreground">
            <Orbit size={48} className="mb-4 opacity-50" />
            <p>
              {searchQuery
            ...
            <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-5 transition-opacity ${isLoading ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
              {projects.map((project, i) => (
                <ProjectCard
                  ...
                  index={i}
                  basePath="/tours"
                  shareResourceType="tour_project"
                />
              ))}
            </div>
            {isLoadingMore && (
        ...
        <DialogContent className="sm:max-w-[420px]">
          <DialogHeader>
            ...
            <DialogDescription className="pl-[52px]">
              Choose a new name for this virtual tour.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="flex items-center gap-3 mb-1">
              ...
              <DialogTitle>
                {deleteModal?.type === 'bulk'
                  ? `Delete ${deleteModal.ids.length} tour${deleteModal.ids.length > 1 ? 's' : ''}?`
                  : 'Delete tour?'}
              </DialogTitle>
            </div>
            <DialogDescription className="pl-[52px]">
              {deleteModal?.type === 'bulk'
                ? `You're about to permanently delete ${deleteModal.ids.length} tour${deleteModal.ids.length > 1 ? 's' : ''} and all their scenes and navigation links. This cannot be undone.`
                : <>You're about to permanently delete <span className="font-medium text-foreground">"{deleteModal?.type === 'single' ? deleteModal.projectName : ''}"</span> and all its scenes and navigation links. This cannot be undone.</>}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="mt-2">
