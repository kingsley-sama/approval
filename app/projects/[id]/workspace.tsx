...
function dbCommentToPin(c: DbComment): Pin {
  return {
    id: c.id,
...
/** Map the server workspace payload to viewer state (no localStorage access — SSR-safe). */
function workspaceToImages(data: WorkspaceData): ImageData[] {
...
export default function ProjectWorkspace({ projectId, initialData, fallbackName }: ProjectWorkspaceProps) {
  ...
  const [imagesState, setImagesState] = useState<ImageData[]>(() => workspaceToImages(initialData));
  const [currentImageId, setCurrentImageId] = useState<string>(initialData.threads[0]?.id ?? '');
  ...
  /** Append locally-queued (not yet synced) comments to the given images. */
  const mergeLocalPins = (images: ImageData[]): ImageData[] =>
    images.map(img => {
      const dbIds = new Set(img.pins.map(p => p.id));
      const localPins = getPendingForThread(img.id)
        .filter(pending => !dbIds.has(pending.localId))
        .map(pendingToPin);
      return localPins.length ? { ...img, pins: [...img.pins, ...localPins] } : img;
    });

  /** Re-fetch the whole workspace in one server call (after uploads, reconnects). */
  const refreshWorkspace = async () => {
    setIsLoading(true);
    try {
      const data = await getProjectWorkspaceData(projectId);
      if (data.projectName) setProjectName(data.projectName);
      const mapped = mergeLocalPins(workspaceToImages(data));
  ...
  // The server already provided threads + comments; on mount we only merge any
  // locally-queued comments (localStorage) and drain stale queue items from a
  // previous session.
  useEffect(() => {
    setImagesState(prev => mergeLocalPins(prev));
    drainQueue(syncCallbacks.onSynced, syncCallbacks.onFailed);
  ...
  // Live-sync edits, moves, and resolves made by other clients. Idempotent for
  // our own echoed updates; attachments and pending state on the local pin are
  // preserved by spreading.
  const handleRealtimeUpdate = useCallback((comment: DbComment) => {
    if (comment.type === 'reply' || comment.parent_comment_id) return;
    setImagesState(prev => prev.map(img => {
      if (img.id !== comment.thread_id || !img.pins.some(p => p.id === comment.id)) return img;
      return {
        ...img,
        pins: img.pins.map(p => p.id === comment.id
          ? {
              ...p,
              content: comment.content,
              x: comment.x_position,
              y: comment.y_position,
              status: comment.status === 'resolved' ? 'resolved' as const : 'active' as const,
              drawingData: comment.drawing_data ?? undefined,
            }
          : p),
      };
    }));
  ...
  const handleRealtimeDelete = useCallback((commentId: string) => {
    setImagesState(prev => {
      if (!prev.some(img => img.pins.some(p => p.id === commentId))) return prev;
      return prev.map(img => ({
        ...img,
  ...
  const { connectionStatus } = useRealtimeComments({
    ...
    onUpdateComment: handleRealtimeUpdate,
    ...
    <div className={`h-screen bg-background text-foreground flex flex-col ${isFullscreen ? 'fixed inset-0' : ''}`}>
      ...
      {connectionStatus === 'disconnected' && (
        <div className="bg-yellow-50 border-b border-yellow-200 px-6 py-1.5 text-xs text-yellow-800 flex items-center justify-between">
          ...
          <button onClick={refreshWorkspace} className="underline underline-offset-2 hover:text-yellow-900">Refresh</button>
        </div>
      )}
