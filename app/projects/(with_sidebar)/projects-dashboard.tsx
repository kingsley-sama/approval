...
import { getProjectsPage, deleteProject, type ProjectListItem, type ProjectSort } from '@/app/actions/projects'
...
export default function ProjectsDashboard({ initialProjects, initialTotal, isAdmin, currentUserId }: ProjectsDashboardProps) {
  ...
  // Drops responses from superseded refetches (e.g. while typing in search)
  ...
  const nextSortOrder: ProjectSort = sortOrder === 'newest' ? 'oldest' : sortOrder === 'oldest' ? 'name' : 'newest'

  const refetchFirstPage = useCallback(async (search: string, sort: ProjectSort) => {
    const seq = ++requestSeq.current
    setIsLoading(true)
    try {
      const { projects: rows, total: newTotal } = await getProjectsPage({ page: 1, search, sort })
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const { projects: rows, total: newTotal } = await getProjectsPage({ page: nextPage, search: searchQuery, sort: sortOrder })
      if (seq !== requestSeq.current) return
      setProjects(prev => {
  ...
  // Server-side search + sort: refetch page 1 when either changes (debounced for typing)
