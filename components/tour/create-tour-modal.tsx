...
import { createTourProject } from '@/app/actions/tour-projects';
...
interface CreateTourModalProps {
  /** Receives the new tour's id so the dashboard can jump straight into the editor. */
  onTourCreated: (projectId: string) => void;
  trigger?: React.ReactNode;
}

const CONCURRENCY = 2;
const MB = 1024 * 1024;
// Scenes are uploaded at full quality (no client compression), so allow a
// generous ceiling for high-resolution equirectangular images.
const MAX_SCENE_SIZE = 50 * MB;
const IMAGE_TYPES = new Set(['image/jpeg', 'image/png', 'image/webp']);

/** Tour-scene validation: equirectangular images only, up to 50 MB, no compression. */
function validateSceneFiles(files: File[]): { accepted: File[]; rejected: { file: File; reason: string }[] } {
  const accepted: File[] = [];
  const rejected: { file: File; reason: string }[] = [];
  for (const file of files) {
    if (!IMAGE_TYPES.has(file.type)) {
      rejected.push({ file, reason: 'Use a JPG, PNG, or WebP equirectangular image.' });
    } else if (file.size > MAX_SCENE_SIZE) {
      rejected.push({ file, reason: `Too large (${(file.size / MB).toFixed(1)} MB). Max ${MAX_SCENE_SIZE / MB} MB.` });
    } else if (file.size === 0) {
...
export default function CreateTourModal({ onTourCreated, trigger }: CreateTourModalProps) {
  ...
  const addFiles = (incoming: File[]) => {
    const { accepted, rejected } = validateSceneFiles(incoming);
  ...
  const handleCreate = async () => {
    if (!projectName.trim()) { setError('Tour name is required'); return; }
    if (selectedFiles.length === 0) { setError('At least one 360° scene image is required'); return; }

    setIsLoading(true);
    setError('');

    try {
      const result = await createTourProject({
        name: projectName,
      ...
      const uploadOne = async (file: File, state: FileUploadState): Promise<void> => {
        ...
        const regResult = await registerTourScene(projectId, file.name, urlResult.storagePath);
        if (!regResult.success) {
          patch(state.id, { status: 'error', error: regResult.error || 'Failed to save scene' });
          return;
        }
        patch(state.id, { status: 'done', progress: 100 });
      };

      // Bounded concurrency — register order also determines scene_index/preview.
    ...
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload one 360° equirectangular photo per room, then link them into a
            walkable virtual tour.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="e.g. Musterstraße 12 — Apartment 3B"
              disabled={isLoading}
            />
          </div>

          <div className="space-y-2">
            <Label htmlFor="tour-desc">Add notes (Optional)</Label>
            <Input
              id="tour-desc"
              value={description}
          ...
          <div className="space-y-2">
            <Label>Scene Images <span className="text-destructive">*</span></Label>
            <div
              ...
              {isUploading ? (
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-2 text-center">
                    {selectedFiles.length} scene{selectedFiles.length !== 1 ? 's' : ''} selected
                  </p>
                </div>
              ) : (
                <div
                ...
                >
                  ...
                  <p className="text-sm text-gray-600 font-medium">Click or drag 360° photos to upload</p>
